...
public interface OrderRepository extends JpaRepository<Order, Integer> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT new org.example.vanphong.dto.TopProductDto(pt.product.name, SUM(oi.quantity)) " +
           "FROM OrderItem oi JOIN oi.productType pt JOIN oi.order o " +
           "WHERE o.status = 'Hoàn thành' " +
           "GROUP BY pt.product.name " +
           "ORDER BY SUM(oi.quantity) DESC")
    List<org.example.vanphong.dto.TopProductDto> findTopSellingProducts(org.springframework.data.domain.Pageable pageable);
}
